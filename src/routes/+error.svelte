...
<div class="error-page">
	<main>
		...
		<article>
			{#if page.status === 404}
				<p>
					<strong>The page you were looking for doesn't exist.</strong> You may have mistyped the
					address or the page may have moved. If you're the application owner check the logs for more
					information.
				</p>
			{:else}
				<p>
					<strong>{page.status} — {page.error?.message ?? 'Something went wrong.'}</strong>
				</p>
			{/if}
		</article>
	</main>
